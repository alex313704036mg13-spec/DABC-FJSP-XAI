"""
ANN + SHAP Analysis for DABC-based FJSP Scheduling Results (MA-only)

This script generates an MA-only dataset using the Discrete Artificial Bee Colony
(DABC) algorithm for the Kacem 4x5 FJSP benchmark, then trains an ANN surrogate
model and applies SHAP to analyze the influence of machine assignment features
on Cmax.

Main outputs:
1. MA-only dataset generated by DABC
2. Best DABC solution and Gantt chart
3. ANN prediction performance
4. Actual vs. predicted Cmax plot
5. MA-only SHAP importance plot
6. MA-only SHAP beeswarm and waterfall plots
7. SHAP importance ranking table
...
def solution_to_record(sol, idx):
    """
    Convert a DABC solution into one MA-only dataset record.
    """
    record = {"ID": idx}
...
def run_dabc_collect_dataset():
    """
    Run DABC once and collect MA-only samples for ANN and SHAP analysis.
    """
...
# ==========================================
# 8. Main Program: MA-only Dataset, ANN, and SHAP
# ==========================================
if __name__ == "__main__":

    # ---------------------------------------------------------
    # Part A: Generate MA-only dataset by DABC
    # ---------------------------------------------------------
    ...
    if len(feature_cols) == 0:
        raise ValueError("No MA_ feature columns were found in the DataFrame.")
